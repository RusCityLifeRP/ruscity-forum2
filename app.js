...
// Твой точный список государственных организаций + криминал
const FACTIONS_STRUCTURE = {
    "gos": [
        ...
        "ЦОДД-Центп организации дорожного движение", 
        "Москва LIVE"
    ],
    "crime": [
        "ОПГ Тамбовское",
...
// Плоский список для выпадающих меню в админке
...
let currentFactionId = ''; // Тут хранится точное имя фракции текстом (например: "МВД")
let currentTopicId = '';
let currentProfileUid = '';
let allUsersDataLocal = {};

// ==========================================
// 3. НАВИГАЦИЯ И ЖЕСТКАЯ ПРОВЕРКА ДОСТУПА
// ==========================================
function backToHome() {
    currentServerId = '';
    currentCategoryId = '';
...
// Главная проверка прав редактирования тем
function hasEditAccess() {
    ...
    // 1. Высшая администрация может редактировать абсолютно всё
    if (ALL_ADMIN_ROLES.includes(currentUser.role)) return true;

    // 2. Лидеры / замы имеют доступ строго к своему серверу и своей фракции
...
function loadForumData() {
    ...
    // Локальная база пользователей для быстрой сверки прав + статистика
...
// Стабильный рендер фракций из массива кода (больше ничего не пропадает)
function openFactions(catId, catName) {
    ...
    if(staticFactions.length === 0) {
        facList.innerHTML = '<div class="empty-notify">Список разделов пуст.</div>';
        return;
    }
...
function openTopicsList(factionId, factionName) {
    ...
    // Скрываем или показываем кнопку создания темы для лидера
...
function openTopic(topicId, isInfoSection = false) {
    ...
    // Проверка доступа к кнопкам редактирования
    if(hasEditAccess()) {
